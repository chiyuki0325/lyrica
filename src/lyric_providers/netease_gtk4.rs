...
#[derive(Clone)]
pub struct NeteaseGtk4LyricProvider {}


impl NeteaseGtk4LyricProvider {
    pub async fn get_lyric_by_metadata(&self, metadata: &Metadata) -> (Vec<LyricLine>, bool) {
        ...
        let music_id = track_id.strip_prefix("/com/gitee/gmg137/NeteaseCloudMusicGtk4/").unwrap().parse::<u64>().unwrap();
    ...
    pub fn is_available_by_metadata(&self, metadata: &Metadata) -> bool {
        metadata.track_id().is_some() && metadata.track_id().unwrap().as_str().starts_with("/com/gitee/gmg137/NeteaseCloudMusicGtk4/")
    }
}
